...
- `202606140013_source_based_problem_refresh.sql`
...
CRON_SECRET=
SCRAPE_SOURCE_URLS=https://puzzling.stackexchange.com/questions/tagged/lateral-thinking,https://example.com/turtle-soup-1
```
...
`SCRAPE_SOURCE_URLS`는 데일리 자동 생성을 위해 필요합니다. 공개 웹 페이지의 `문제:`/`정답:` 형식 후보와 Puzzling Stack Exchange `lateral-thinking` 태그 API 후보를 수집하고, 원문 복제가 아니라 서비스용 문제로 재구성한 뒤 `source='Web'`, `source_url`을 저장합니다. 접근 실패, HTML 형식 불일치, 후보 부족 시 해당 URL은 건너뛰며, 최종 후보가 없으면 AI 신규 생성으로 fallback하지 않고 실패 이력을 남깁니다.

...
export function ResultDialog({ session }: { session: GameSession | null }) {
  ...
  async function shareResult() {
    const text = `Turtle Soup AI에서 '${session?.problem.title}' 문제를 ${session?.questionCount}개 질문으로 해결했어요.`;
    try {
      if (navigator.share) {
        await navigator.share({ title: "Turtle Soup AI", text, url: window.location.origin });
      } else {
    ...
    <dialog ref={ref} className="m-auto w-[min(92vw,460px)] rounded-none bg-transparent p-0 backdrop:bg-black/70">
      <section className="pixel-panel max-h-[88dvh] overflow-y-auto p-6 text-[#deded8]">
        ...
        <div className="space-y-5 leading-7">
          ...
          <div className="grid grid-cols-3 gap-3 border-t border-white/8 pt-5 text-center text-sm">
            ...
            <div className="rounded-lg border border-[#22222e] bg-[#0e0e14] p-3"><strong className="block text-sm text-[#b89040]">{session.problem.difficulty}</strong><span className="muted text-xs">난이도</span></div>
          </div>
        </div>
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button className="pixel-button flex items-center justify-center gap-2" type="button" onClick={() => window.location.assign(`/game/${session.problemId}`)}><RotateCcw aria-hidden="true" size={18} />다시 풀기</button>
